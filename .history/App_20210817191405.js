...
export default class App extends Component {
  ...
  render(){
    ...
    async function getMygames({navigation}){
      let keys = await getAllKeys()
      let myGames =[]
      keys.forEach( key => {
        let id = key.slice(1,-1)
        for(let i in Games){
          if(Games[i]["gameID (S)"]==id){
            //console.log(Games[i]["gameID (S)"]==id)
      ...
      myGameData.set
      return(makeElements(myGames,{navigation}))
    }

    componentDidMount(){
      getMygames({navigation})
        .then(this.setState({myGameData: getMygames({navigation})}))
    };
    //Screens
    ...
    function MyScreen({navigation}){
      let output
      let mySavedGames = getMygames({navigation}).then(output = <Rows data={mySavedGames} textStyle={styles.text}/>)

      return(
        <View style={styles.container}>
          <ScrollView>
            <Text>Your Saved Games</Text>
            <Button onPress={()=>getMygames({navigation})} title="Press Me"/>
            <Table borderStyle={{borderWidth: 2, borderColor: '#c8e1ff'}}>
            </Table>
          </ScrollView>
        </View>
