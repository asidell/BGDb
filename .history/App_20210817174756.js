...
export default class App extends Component {
  ...
  render(){
    ...
    function MyScreen({navigation}){
      let keys = getAllKeys()
      let data = []
      for(let k =0; k<keys["_W"].length;k++){
        console.log(keys["_W"][k].slice(1,-1))
        if(isNumeric(keys["_W"][k].slice(1,-1))){
          console.log(keys["_W"][k].slice(1,-1))
        }
      }
      ...
      return(
        <View style={styles.container}>
          <ScrollView>
            ...
            <Button onPress={()=> console.log(keys["_W"][2].slice(1,-1))} title="Press Me"/>
            <Table borderStyle={{borderWidth: 2, borderColor: '#c8e1ff'}}>
