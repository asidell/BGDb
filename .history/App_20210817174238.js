...
export default class App extends Component {
  ...
  render(){
    ...
    function MyScreen({navigation}){
      let keys = getAllKeys()
      let data = []
      console.log(typeof(keys["_W"]))
      for(let k in keys["_W"]){
        if(isNumeric(keys["_W"][k].slice(3,-3))){
          console.log(keys["_W"][k])
        }
      }
      let mySavedGames = makeElements(Games,{navigation})
      return(
        <View style={styles.container}>
          <ScrollView>
            <Text>Your Saved Games</Text>
            <Button onPress={()=> console.log(keys["_W"].length)} title="Press Me"/>
            <Table borderStyle={{borderWidth: 2, borderColor: '#c8e1ff'}}>
